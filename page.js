...
export default function Home(){
 ...
 return <main>
  ...
  <section id="results" className="content"><div className="headline"><div><span className="small">{favoritesOnly?'MES FAVORIS':searched?'OFFRES D’EMPLOI':'OFFRES RÉCENTES'}</span><h2>{loading?'Recherche en cours…':favoritesOnly?`${filteredJobs.length} favori${filteredJobs.length!==1?'s':''}`:searched?`${filteredJobs.length} offre${filteredJobs.length!==1?'s':''} trouvée${filteredJobs.length!==1?'s':''}`:'Les dernières offres publiées'}</h2></div>{favoritesOnly&&<button className="backResultsTop" onClick={backToResults}>← Revenir aux offres</button>}</div>{!favoritesOnly&&searched&&!loading&&!error&&(sourceCounts.franceTravail||sourceCounts.jooble||sourceCounts.adzuna)?<div className="sourceSummary"><span>France Travail <b>{sourceCounts.franceTravail}</b></span><span>Jooble <b>{sourceCounts.jooble}</b></span><span>Adzuna <b>{sourceCounts.adzuna}</b></span></div>:null}
   ...
   <div className="offers">{filteredJobs.map((o,index)=>{const internalUrl=o.sourceKey==='francetravail'?`/offres/france-travail/${String(o.id).replace(/^ft-/,'')}`:null;return <div className="offerWithAd" key={o.id}><article className="card"><button className={favoriteIds.has(o.id)?'heart active':'heart'} aria-label="Ajouter aux favoris" onClick={()=>toggleFavorite(o)}>♥</button><div className="icon">💼</div><div className="cardMain"><div className="cardTop"><div><h3>{internalUrl?<a href={internalUrl} onClick={()=>track('job_view',{source:o.sourceKey||'francetravail',job_title:String(o.title||'').slice(0,100),city:String(o.city||'').slice(0,80),placement:'title'})}>{o.title}</a>:o.title}</h3><p>{o.company}</p></div><span className="distance">📍 {o.city}</span></div><div className="meta">{o.source&&<span className="sourceBadge">{o.source}</span>}{o.sourceKey==='adzuna'&&<a href="https://www.adzuna.fr/" target="_blank" rel="noopener noreferrer" className="adzunaAttribution">Jobs by Adzuna</a>}{o.contract&&<span>{o.contract}</span>}{o.salary&&<span>{o.salary}</span>}{o.created&&<span>Publié le {formatDate(o.created)}</span>}</div>{o.description&&<p className="previewText">{o.description.slice(0,180)}{o.description.length>180?'…':''}</p>}<div className="actions">{internalUrl?<><a className="jobButton" href={internalUrl} onClick={()=>track('job_view',{source:o.sourceKey||'francetravail',job_title:String(o.title||'').slice(0,100),city:String(o.city||'').slice(0,80),placement:'button'})}>Voir l’offre sur Jobelyo</a><span>Consultez l’offre puis postulez sur le site officiel</span></>:<><button className="detailButton" onClick={()=>{track('job_view',{source:o.sourceKey||o.source||'unknown',job_title:String(o.title||'').slice(0,100),city:String(o.city||'').slice(0,80),placement:'modal'});setSelected(o)}}>Voir les détails</button><a className="jobButton secondary" href={o.url} target="_blank" rel="noopener noreferrer" onClick={()=>track('apply_click',{source:o.sourceKey||o.source||'unknown',job_title:String(o.title||'').slice(0,100),city:String(o.city||'').slice(0,80),placement:'card'})}>Postuler</a><span>Candidature externe • aucun compte Jobelyo requis</span></>}</div></div></article>{!favoritesOnly&&searched&&index===3&&<AdBanner slot={process.env.NEXT_PUBLIC_ADSENSE_SLOT_1}/>} {!favoritesOnly&&searched&&index===6&&<LingzioPartner/>} {!favoritesOnly&&searched&&index===11&&<AdBanner slot={process.env.NEXT_PUBLIC_ADSENSE_SLOT_2}/>}</div>})}
  ...
  <section aria-labelledby="jobelyo-guide" style={{maxWidth:1100,margin:'0 auto',padding:'42px 20px'}}>
   <div style={{background:'#fff',borderRadius:20,padding:'30px',boxShadow:'0 8px 30px rgba(0,0,0,.06)'}}>
    <span className="small">BIEN CHERCHER SON EMPLOI</span>
    <h2 id="jobelyo-guide">Jobelyo vous aide à trouver un emploi plus facilement</h2>
    <p>Jobelyo permet de rechercher des offres d’emploi par métier, ville et rayon. Vous pouvez comparer les annonces, consulter leurs détails et accéder au site prévu pour déposer votre candidature.</p>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:18,marginTop:24}}>
     <div><h3>1. Définissez votre recherche</h3><p>Indiquez le métier ou les mots-clés qui vous intéressent, puis choisissez une ville et un rayon adapté à vos déplacements.</p></div>
     <div><h3>2. Comparez les offres</h3><p>Regardez le contrat, le lieu, l’entreprise, la date de publication et les informations disponibles avant de choisir une annonce.</p></div>
     <div><h3>3. Préparez votre candidature</h3><p>Adaptez votre CV et votre message à l’offre. Vérifiez les informations demandées avant d’envoyer votre candidature.</p></div>
    </div>
    <div style={{marginTop:24,paddingTop:22,borderTop:'1px solid #e5e7eb'}}>
     <h3>Conseils pratiques</h3>
     <p>Pour aller plus loin, consultez nos <a href="/conseils-emploi">conseils pour la recherche d’emploi</a> et découvrez <a href="/a-propos">comment fonctionne Jobelyo</a>. La recherche d’offres reste accessible sans créer de compte.</p>
    </div>
   </div>
  </section>
  ...
  <footer><div className="brand">Job<span>elyo</span></div><p>Recherche d’offres en France • candidature sans compte Jobelyo.</p><nav className="footerLinks" aria-label="Informations"><a href="/conseils-emploi">Conseils emploi</a><a href="/a-propos">À propos</a><a href="/contact">Contact</a><a href="/mentions-legales">Mentions légales</a><a href="/confidentialite">Confidentialité</a></nav></footer>
